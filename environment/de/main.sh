...
function SearchForInstalled {
  ...
  for index in ${!DesktopSessions[*]}; do
    [[ $(sh "$functions" _isInstalled "${DesktopSessions[index]}") = 0 ]] && Installed+=("${Available[index]}")
  done
}
  ...
  if ! [[ $KEY_PRESS = $'\e' ]]; then
    if [[ $KEY_PRESS =~ ^[0-9]+$ ]]; then
      if [[ $KEY_PRESS -le ${#Available[@]} ]]; then
        ...
        if [[ -e "$Source_Path/${Packages[$KEY_PRESS]}/custom.sh" ]]; then
          sh "$Source_Path/${Packages[$KEY_PRESS]}/custom.sh"
        fi
      fi
    fi
  else
